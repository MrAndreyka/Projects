...
/*wchar_t* ToString(char* mbString)
{
	...
	mbstowcs_s(&len, ucString, len, mbString, len);
	return ucString;
}*/
...
bool StaticMesh::m_loadMS3DFile(const wchar_t* Filename)
{
	...
	if (!m_LoadTextures(pMS3DMaterials[0].texture))
		return false;
	...
	_debug(Log::Get()->Debug(("Load vertices: "+ std::to_string(VertexCount)).c_str()));
	_debug(Log::Get()->Debug(("Load indices: "+ std::to_string(m_indexCount)).c_str()));
...
bool StaticMesh::m_LoadTextures(const LPCSTR textureFilename)
{

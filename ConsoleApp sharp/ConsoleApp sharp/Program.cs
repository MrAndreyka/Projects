...
namespace ConsoleApp_sharp
{
    partial class Program
    {
            ...
            public static string TryParse(Txt_Panel res, MyTree val, Action<List<ITxt_null>, string, int> GetSurf, Func<string, bool> GetBool = null)
            {
                ...
                foreach (var x in val)
                {
                    ...
                    { 
                        ...
                        er = Txt_Panel.TryParse(p, x, GetSurf, GetBool);
        ...
        public class Sel_Item : Item_sel
        {
            ...
            public bool Include(Item_sel obj)=> (Type > 0 ? (Type == obj.Type) : base.Equals(obj)) != invert;
            public override string ToString()=> $"{(invert ? "!" : "")}{(Name)}";
        }
        ...
        public class Sel_Group : List<Sel_Item>, ISaving
        {
            ...
            public bool Fits(Sel_Item val) => Find(x => x.Equals(val)) != null;
            public bool Equals(Item_sel val) => Find(x => x.Include(val)) != null;
            ...
            {
                if (res == null) res = new MyTree();
                ForEach(x => res.Add(new MyTree(x.Type.ToString(), x.ToString())));
                return res;
            }
            public static Sel_Group Parse(MyTree val)
            {
                ...
                foreach (var v in val)
                    res.Add(new Sel_Item(byte.Parse(v.Param), v.Name.Part('!', true), v.Name[0] == '!'));
                return res;
            }
        ...
        public class MyTree : List<MyTree>
        {
            ...
            public MyTree() { }
            public MyTree(string param, string name = null) { Param = param; Name = name; }
            ...
            public void Add(string name, string param = null) { Add(new MyTree(name, param)); }
 ...
 []
  ...
  [Surface=True]
   ...
   2=0";
        ...
        static IEnumerator<bool> Sequence() {
            int i = 0;
            Console.Write(i++);
            yield return false;
        ...
        {
            IEnumerator<bool> _stateMachine = Sequence();

            int g = 30;
            while (g > 0)
                Console.WriteLine("\tG:{1} {0}", _stateMachine.MoveNext(), g--);

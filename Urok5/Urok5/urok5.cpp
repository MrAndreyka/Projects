...
struct SimpleVertex
{
	...
	int indNorm;	 // Нормаль вершины
};

struct VertexData
{
	XMFLOAT3 Normal;	 // Нормаль вершины
	XMFLOAT4 Color;	 // Координаты точки в пространстве
};
...
ID3D11Buffer*			g_pCBNeverChanges = NULL;	// Константный буфер2
...
HRESULT InitGeometry()
{
    ...
    if(FAILED( hr ))
    {
        MessageBox( NULL, L"Невозможно скомпилировать файл FX (VS). Пожалуйста, запустите данную программу из папки, содержащей файл FX.", L"Ошибка", MB_OK );
        return hr;
    }
	...
	pVSBlob->Release();
    if (FAILED(hr)) {
		MessageBox(NULL, L"Ошибка cоздания шаблона вершин", L"Ошибка", MB_OK);
		return hr;
	}
    ...
    if( FAILED( hr ) )
    {
        MessageBox( NULL, L"Невозможно скомпилировать файл FX (PS). Пожалуйста, запустите данную программу из папки, содержащей файл FX.", L"Ошибка", MB_OK );
        return hr;
    }
	...
	pPSBlob->Release();
	if (FAILED(hr)) {
		MessageBox(NULL, L"Ошибка cоздания пиксельного шейдера", L"Ошибка", MB_OK);
		return hr;
	}
	...
	if (FAILED(hr)) {
		MessageBox(NULL, L"Ошибка cоздания константного буфера", L"Ошибка", MB_OK);
		return hr;
	}
	...
	if (FAILED(hr)) {
		MessageBox(NULL, L"Ошибка cоздания буфера цветов", L"Ошибка", MB_OK);
		return hr;
	}
